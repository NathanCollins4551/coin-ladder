...
const ProfitLossIcon = ({ isProfit }: { isProfit: boolean }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={`h-8 w-8 ${isProfit ? 'text-green-500' : 'text-red-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        {isProfit ? (
...
export default async function HomePage() {
    ...
    return (
        <div className="space-y-10 bg-gray-50/50 p-4 sm:p-6 lg:p-8 rounded-lg">
            {/* Header */}
            <header>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 tracking-tight">Dashboard</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    {/* Portfolio Summary */}
                    <section>
                        <h2 className="text-2xl font-bold text-gray-800 mb-4">Portfolio Snapshot</h2>
                        {portfolio.length > 0 ? (
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                    <div className="flex items-center p-4 bg-gray-50 rounded-xl space-x-4">
                                        <div className="bg-blue-100 p-3 rounded-full"><ValueIcon /></div>
                                        <div>
                                    ...
                                    <div className="flex items-center p-4 bg-gray-50 rounded-xl space-x-4">
                                        <div className="bg-green-100 p-3 rounded-full"><InvestedIcon /></div>
                                        <div>
                                    ...
                                    <div className="flex items-center p-4 bg-gray-50 rounded-xl space-x-4">
                                        <div className={`${totalProfit >= 0 ? 'bg-green-100' : 'bg-red-100'} p-3 rounded-full`}><ProfitLossIcon isProfit={totalProfit >= 0} /></div>
                                        <div>
                                            <h3 className="text-sm font-medium text-gray-500">Overall P&L</h3>
                                ...
                                <Link href="/portfolio" className="block text-center mt-6 text-sm font-medium text-blue-600 hover:underline">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
                                ...
                                <Link href="/cryptocurrencies" className="inline-block mt-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition-colors">Start Trading!</Link>
                            </div>
                        )}
                    </section>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <ul className="space-y-5">
                                {newsArticles.slice(0, 4).map((article: NewsArticle) => (
                                    <li key={article.title} className="border-b border-gray-100 pb-5 last:border-0 last:pb-0">
                                        <a href={article.link} target="_blank" rel="noopener noreferrer" className="group flex items-start space-x-4">
                                            ...
                                            <div className="flex-grow">
                                                <h3 className="font-semibold text-gray-800 group-hover:text-blue-600 transition-colors leading-tight">{article.title}</h3>
                                                <p className="text-xs sm:text-sm text-gray-500 mt-1">{article.source} &middot; {formatDate(article.date)}</p>
                                            </div>
                                        </a>
                                    </li>
                                ))}
                            </ul>
                            <Link href="/news" className="block text-center mt-6 text-sm font-medium text-blue-600 hover:underline">
                                See all news &rarr;
                            </Link>
                        </div>
                    </section>
                                ...
                                {recentTrades.slice(0, 8).map((trade: Trade, index: number) => (
                                    <li key={index} className="flex items-center justify-between">
                                        <div className="flex items-center">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${trade.type === 'BUY' ? 'bg-green-100' : 'bg-red-100'}`}>
                                                <span className={`text-lg font-bold ${trade.type === 'BUY' ? 'text-green-600' : 'text-red-600'}`}>
                                                    {trade.type === 'BUY' ? 'B' : 'S'}
                                                </span>
                                            </div>
                                            <div>
                                                <p className="font-bold text-gray-900">{trade.coin_symbol.toUpperCase()}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-semibold text-gray-800">{trade.crypto_amount.toFixed(5)}</p>
                                            <p className="text-sm text-gray-500">{formatCurrency(trade.fiat_amount)}</p>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                            <Link href="/my-trades" className="block text-center mt-6 text-sm font-medium text-blue-600 hover:underline">
